...
\begin{example}[Constrained Pizza Selection Problem]\label{example:pizza}
    Peter, Alina, Ronald and Sergey decided to order three pizzas in a trattoria: margaritha ($m$), veggie ($g$) and meat-lovers ($l$) -- each pizza can be shared by at most two people and everyone specified pizzas they can eat, togehter with their preferrences for the selected pizzas on the scale from one to ten. Can we help them to share the pizzas in an optimal way?
    ...
    Then, we need to model the fact that all pizza being eaten by someone, since we do not like wasting food:
    \begin{equation}
...
Therefore, when we write a first order model for a problem, we can always map it to the language of FO(.), furthermore provided language extensions make modeling of relational problems more natural and convenient
