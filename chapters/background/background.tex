...
\section{Pattern Mining}\label{sec:prelim}
\subsection{Pattern Types} 
\leanparagraph{Itemsets}
...
t_i, t_i \in D\}$ and we refer to $|D_{\alpha}|$ as the \emph{support (frequency)} of $\alpha$ in $D$, written $\mi{sup}(\alpha)$. The \emph{relative frequency} of $\mi{\alpha}$ in $D$ refers to the ratio between $\mi{sup}(\alpha)$ and $\mi{|D|}$. %Moreover, we 
The \emph{cardinality} (or \emph{size}) $|\alpha|$ of an itemset $\alpha$ is %defined as 
...
\leanparagraph{Sequences} A \emph{sequence} is an ordered set of items $\tuple{s_1,\dotsc,s_n}$.
...
Given two sequences $S = \tuple{s_1,\dotsc, s_n}$ and $S'=\tuple{s_1',\dotsc, s_m'}$, of %length 
...
\begin{definition}[Frequent Sequence]\label{def:frseq} For a sequential dataset $D=\{S_1,\dotsc, S_n\}$ and a frequency threshold $\sigma \geq 0$, a sequence $S$ is \emph{frequent} in $D$ if $\mi{sup}(S)\geq \sigma$. %, where $\sigma \geq 0$ is a user-specified minimum \emph{support}. 
\end{definition}
...
In data mining, constraints are typically specified by the user to encode domain background knowledge. \cite{DBLP:conf/cpaior/NegrevergneG15} distinguish four types of constraints : constraints 1) over the pattern (e.g., restriction on its %length
...
The order in which constraints are applied influences the solution set \parencite{DBLP:journals/kais/BonchiL06}. %In this work w
%We take the same view a
As in \parencite{DBLP:journals/kais/BonchiL06} in this work we %and %aim at 
apply %ing 
...
\subsection{Relational Pattern Mining}
