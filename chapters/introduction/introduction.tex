...
\pubrevend
\sergey{some words on how it affects different types of problems}


\section{Types of mining problems}
\sergey{insert: predictive models, pattern mining, constraint learning
and BMF here}

\sergey{explain that are important types of problems in data mining}

\subsection{Predictive models} Classical machine learning algorithms such as the induction of decision trees
...
\sergey{ we do not do predicitve models and now going to describe a
relational problem that we do}
...
\sergey{say that it's of great importance to us}
...
In general not in this example, these properties of interconnectedness and expressivity of the constraints make the problem hard to model using traditional propositional approaches, while this can be clearly seen as a first order problem. This gives us confidence that for a number of important problems (such as recovering formulae in spreadsheets, factorizing relational data and mining frequent structures in relational data) it is essential to look into richer knowledge representation languages for machine learning.

\subsection{Relational Pattern Mining}
\sergey{explain graph mining under constraints refer to our graph
mining paper with Matthias here}
...
an ideal candidate for our general solver in the schema in Figure
\ref{fig:declarative_schema}, since both of them support relational
data natively and can solve problems declaratively based on a form of
first order logic. We often refer to both paradigms (and to systems,
...
Fourth, as we have just indicated, our solutions are declarative in its nature, as indicated in Figure \ref{fig:declarative_schema} Once we modify the problem by adding an extra constraint or feature, we do not have to modify algorithm completely (like this often happens in the imperative approach \parencite{gspan,clospan}).
